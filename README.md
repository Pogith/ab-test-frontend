...
- README 작성
- Test code 작성
  - Frontend : [testing-library](https://testing-library.com/docs/react-testing-library/intro/)
  - Backend : [Mocha](https://mochajs.org/) / [Supertest](https://www.npmjs.com/package/supertest) / [Chai](https://www.chaijs.com/)
...
- CORS
  ...
  제가 개발하고 있는 client와 server 둘만을 단순히 연결시키는 것이 아니라 사용자의 웹 사이트에서도 저의 server와 통신하여 데이터를 저장시켜야 하므로 CORS 에러가 발생하였습니다.
  CORS 에러는 client에서 자신이 보낸 요청의 origin과 서버가 보내준 Acess-Cross-Allow-Origin을 비교하여 차단할지 말지 결정하고 유효하지 않다면 그 응답을 사용하지 않고 버리게 되어 발생하게 됩니다.

  응답을 받는 서버 쪽에서 해결해야 하므로 Node.js 패키지인 cors를 사용하여 해결할 수도 있고, server의 응답 헤더에 Access-Control-Allow-Origin 값으로 모든 출처를 허용한다는 의미인 *를 설정하여 주면 CORS 정책 위반으로 인한 제약을 받지 않아 CORS 에러를 해결할 수 있었습니다.
  하지만 바꿔 생각하면 정체도 모르는 알 수 없는 출처에서 오는 요청까지 모두 허용한다는 의미도 되므로 보안이 허술해진다는 점을 알고 있어야 하며 가급적이면 출처를 명시해 주는 것이 좋다는 것을 알고 있어야 합니다.

  매번 단순히 제가 개발하고 있는 client와 server에서만 요청과 응답을 주고받았기 때문에 크게 문제가 없었지만 이번에 다른 origin에서의 요청들이 많은 개인 프로젝트를 진행하면서 CORS 관련 에러를 경험하면서 보다 정확하게 어떤 개념인지 확실하게 알 수 있었고, client의 요청 헤더와 backend의 응답 헤더의 내용과 흐름에 대해서도 보다 정확하게 이해할 수 있는 프로젝트였습니다.

- Cookie 설정

  방문자의 재방문을 판단하기 위해 cookie를 server에서 보내주는 로직을 작성하였는데, 로컬 환경에서는 cookie를 잘 주고받았었지만 배포하고 난 뒤에 테스트를 해보니 server에서는 cookie를 주는데 해당 테스트 웹사이트에서 cookie를 저장하지 않았고, 또한 요청 헤더에 cookie가 없다는 문제를 발견하였습니다.

  유저의 테스트 웹사이트에 보내주는 js 파일에 fetch를 사용하여 옵션에 cookie를 주고받을 수 있게 설정을 해주었는데, 정확한 원인을 조사해 보니 **"2020년 2월에 출시되는 Chrome 80버전부터 새로운 cookie 정책이 적용되어서 SameSite 값이 없는 cookie를 SameSite=Lax cookie로 취급한다"라는 것을 알게 되었습니다.**

  제 코드에 cookie를 보내줄 때, SameSite를 적지 않았고 기본적으로 None으로 설정한 것과 동일하게 동작할 거라 생각했지만 위의 내용을 통해 Chrome 80버전 이후에는 SameSite 속성 설정 없는 쿠키는 “SameSite=Lax”로 명시한 것과 동일하게 작동하게 되므로 “Strict”와 유사하게 SameSite가 아닌 경우 cookie 전송이 차단되는 것입니다. **또한 “SameSite=None”을 사용하기 위해서는 해당 cookie에 “Secure”을 필수적으로 추가**해주어야 된다고 조사를 하며 알게 되었습니다.

  위와 같은 옵션을 추가해 주니 결과적으로 저의 server에서 cookie를 주면 유저의 테스트 웹사이트에서 보내준 cookie를 저장하고 이벤트가 발생하였을 경우, 저장된 cookie가 요청 헤더에 담기는 것을 볼 수 있었습니다.
  이 문제를 해결하면서 작은 부분이라도 항상 정책이나 변경사항 등에 대해서 빠르게 파악하고 대응할 수 있어야 하고, 문제들을 어떻게 해결해야 되는지에 대해서도 것을 배울 수 있는 시간이었습니다.
...
- 소감
    ...
    팀 프로젝트는 팀원들과 소통을 할 수 있기 때문에 문제 해결이나 계획에 대해 보다 원활하게 진행할 수 있지만, 개인 프로젝트는 하나부터 열까지 모두 다 개인 스스로 계획 및 기획, 기능 개발, 리팩토링, css, 문제 해결 등 모든 부분들을 혼자 헤쳐 나가야 하기 때문입니다.

    개인 프로젝트를 하면서 부담감이 더욱 커지고 과연 “내 실력으로 프로젝트를 시작부터 끝까지 잘 마무리할 수 있을까?”라는 의구심이 들고 저 자신이 많이 위축되었지만, 성장하는 과정에서는 당연히 겪는 과정이고, 최대한 계획한 부분들을 완성시켜보자는 생각으로 저 자신을 매일매일 다독이며 계획대로 끝내기 위해 열심히 한 결과, 지금까지 올 수 있었던 것 같고, 마음가짐의 중요성에 대해 알게 되는 좋은 경험이었습니다.

    또한 이번 개인 프로젝트를 통해 보다 좋은 개발자가 되기 위해서는 개발 실력도 중요하지만 전반적인 개발 흐름에 대해서 잘 알고 적재적소 사용할 줄 알아야 하고, 문제 해결을 위해 끊임없이 파고들어야 하는 자세를 가져야 한다는 것을 알게 되었습니다.
